---
title: "About – Christian Shannon"
---

# About Me

I am Christian Shannon, a Data Scientist based in Papillion, Nebraska, with 15+ years of experience delivering data-driven solutions at Lincoln Financial Group.   
I bring a blend of technical depth and business understanding, moving from frontline operations into senior analytics and consulting roles. 

My work centers on turning raw data into clear stories, building dashboards and reports that answer real business questions, and strengthening data governance so that stakeholders can trust the information they use.   
I am currently pursuing a Bachelor of Science in Data Science at Bellevue University, further deepening my analytical and statistical toolkit. 

---

## Professional Summary

Over my career, I have led the design, development, and deployment of dashboards and data products that help leaders make informed strategic decisions.   
I regularly leverage Python, SQL, ETL processes, and visualization tools like Tableau and Dataiku to deliver accurate, visually compelling reports. 

I am known for mentoring others, driving process improvements, and leading change initiatives that save significant time and reduce manual effort across teams.   
I work closely with IT, data governance, and business stakeholders to optimize data management processes and resolve data integrity challenges. 

---

## Skills & Tools

**Languages & Tools**

- SQL  
- Dataiku  
- Tableau
- Python
- ETL tools (including Informatica PowerCenter) 

**Data & Analytics**

- Data analysis and data mining  
- Data visualization and dashboard design  
- Report optimization and performance analysis  
- Root cause analysis 

**Data & Business**

- Data governance and data quality  
- Data modeling  
- Business metrics and analysis  
- Cross-functional collaboration and stakeholder communication 

---

## Experience Snapshot

**Sr. Analyst / Sr. Consultant – Data Analytics & Business Metrics**  
*Lincoln Financial Group | 2022–Present*   

- Lead dashboard design and deployment for business leaders.  
- Use SQL, ETL, and visualization tools to build accurate, stakeholder-ready reports.  
- Partner with IT and enterprise teams to improve data availability, accuracy, and quality.  
- Troubleshoot data integrity issues that impact enterprise reporting and governance. 

**Analyst, Application Development**  
*Lincoln Financial Group | 2019–2022*   

- Designed and optimized automated ETL workflows using Informatica PowerCenter.  
- Led requirements gathering, design sessions, and code walkthroughs with stakeholders.  
- Helped define data models, best practices, and development standards. 

**Data Governance Consultant**  
*Lincoln Financial Group | 2017–2019*   

- Analyzed processes and metrics to identify and mitigate data quality risks.  
- Built SQL-based data profiles to ensure reliable data assets.  
- Helped develop data governance policies and scalable data modeling. 

*(You can continue adding earlier roles more briefly or keep these as a “snapshot” to keep the page focused.)* 

---

## Education

**Bellevue University**  
Bachelor of Science (BS), Data Science (in progress)  
Bellevue, NE | 2024–Present 

**Metropolitan Community College**  
Associate of Science (AS), Information Technology  
Omaha, NE | 2016–2018 

---

## Leadership & Involvement

- Mentor  
- Alation Subject Matter Expert (SME)  
- Data Advisory Group (DAG) Representative 

I enjoy mentoring colleagues, sharing best practices in data governance and analytics, and helping teams adopt a more data-driven mindset. 
